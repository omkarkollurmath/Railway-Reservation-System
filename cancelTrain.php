...
?>
  ...
  <div class="container">
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
             ...
             <input type="text" name="number" placeholder="Enter 5 digit number" style="width: 300px; height: 30px;" value="<?php echo $number;?>" maxlength="5">
...
$st = "SELECT count(*) as count from train_pnr_demo2 t  where t.TrainId = (select s.TrainId from traininfo s where s.TrainNo = '".$number."')";
...
$sql = "DELETE FROM traininfo  WHERE TrainNo = '".$number."'";

$result = $conn->query($sql);

     if (mysqli_affected_rows($conn) > 0) {
        
        echo "<label class='showResult' style='color : green; font-size: 20px;'><br><br><br>Train Deleted Successfully!</label>";
    }else{
        echo "<label class='showResult' style='color : red;  font-size: 20px;'>Error No record for Train Found</label>";
    }
  ...
  echo"<script>
  alert('You cannot delete the Train since it is booked!!');
  </script>";
}
mysqli_close($conn);
}
